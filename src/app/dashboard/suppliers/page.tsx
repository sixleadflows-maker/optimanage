...
import { suppliers, purchaseOrders } from "@/lib/mock";
import { formatCurrency, formatDate } from "@/lib/utils/format";
import { useApp } from "@/lib/context";
import { Truck, Package, CheckCircle, Clock, FileText } from "lucide-react";
          ...
          {suppliers.map((s) => (
            <div key={s.id} className="glass-card p-5">
              ...
              <div className="space-y-1.5 text-xs text-muted-foreground">
                ...
                <p>✉️ {s.email}</p>
                <p>📍 {s.address}</p>
                <p className="font-mono text-[10px]">GST: {s.gst}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-4">
          {purchaseOrders.map((po) => (
            <div key={po.id} className="glass-card p-5">
              ...
              {po.status !== "Received" && (
                <button onClick={() => showToast(`Stock received for ${po.poNumber} (demo)`, "success")}
